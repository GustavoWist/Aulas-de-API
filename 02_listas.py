...
#ou seja, soma todos os números e divide pela quant
def media(lista):
    soma = 0
    for n in lista:
        soma += n
...
def cria_naipe(naipe):
    ...
    cartas = [valor + naipe for valor in valores]
    return cartas 
...
def  liquidacao(precos,descontos):
    precos_com_desconto = [preco - desconto for preco, desconto in zip(precos, descontos)]
    
    return precos_com_desconto

def juntar():
    nomes = ['Gustavo','Beatriz','Giane','Wusleid','Mafalda']
    idades = [19, 18, 53, 51, 72]
